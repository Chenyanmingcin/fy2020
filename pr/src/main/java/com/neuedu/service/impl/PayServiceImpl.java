...
*/
    ...
    public ServerResponse trade_precreate(OrderVO orderVO) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        String outTradeNo = String.valueOf(orderVO.getShippingId());

        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店当面付扫码消费”
        String subject = "cx品牌cx门店当面付扫码消费";
        ...
        String body = "购买商品"+orderVO.getOrderItemVOList().size()+"件共"+
                orderVO.getPayment()+"元";
        ...
        switch (result.getTradeStatus()) {
            ...
            case FAILED:
                log.error("支付宝预下单失败!!!");
                return ServerResponse.serverResponseByFail(StatusEnum.ALIPAY_ADD_FAIL.getStatus(),StatusEnum.ALIPAY_ADD_FAIL.getDesc());
